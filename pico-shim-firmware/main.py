...
# Encoders (rising edge count on Channel A)
ENC_LEFT_A  = Pin(18, Pin.IN, Pin.PULL_UP)
ENC_LEFT_B  = Pin(19, Pin.IN, Pin.PULL_UP)
ENC_RIGHT_A = Pin(16, Pin.IN, Pin.PULL_UP)
ENC_RIGHT_B = Pin(17, Pin.IN, Pin.PULL_UP)
    ...
    while True:
        ...
        if abs(current_pitch) > ANTI_TUMBLE_ANGLE and imu_ok:
            ...
            if pid_active and imu_ok:
                ...
                left_speed = apply_deadband(pid_output + yaw_offset, MOTOR_DEADBAND)
                right_speed = apply_deadband(pid_output - yaw_offset, MOTOR_DEADBAND)
            ...
            # ---- DIRECT DRIVE (no IMU) ----
                ...
                # Map pitch directly to speed: pitch/30 = normalized speed
                base_speed = target_pitch / 30.0
                base_speed = max(-1.0, min(1.0, base_speed))

                left_speed = apply_deadband(base_speed + yaw_offset, MOTOR_DEADBAND)
                right_speed = apply_deadband(base_speed - yaw_offset, MOTOR_DEADBAND)
        ...
        if time.ticks_diff(now, last_telemetry) >= TELEMETRY_INTERVAL_MS:
            ...
            telemetry_str = (f"P:{current_pitch:+6.1f} T:{target_pitch:+6.1f} "
                             ...
                             f"E:{enc_left_count}/{enc_right_count}")
            print(telemetry_str)
